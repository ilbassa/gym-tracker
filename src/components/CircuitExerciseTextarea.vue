...
<template>
  <div class="field">
    ...
    <textarea ref="textarea" :id="id" class="field__control" :class="{'field__control--error':error}" :value="modelValue" :rows="rows" :aria-invalid="Boolean(error)" :aria-describedby="error?`${id}-error`:matches.length?`${id}-suggestions`:undefined" @input="emit('update:modelValue',($event.target as HTMLTextAreaElement).value)" />
    <span v-if="error" :id="`${id}-error`" class="field__error">{{error}}</span>
    <div v-if="matches.length" :id="`${id}-suggestions`" class="suggestions">
      <span>Esercizi già usati</span>
      <div><button v-for="name in matches" :key="name" type="button" @click="choose(name)">{{name}}</button></div>
    </div>
  </div>
</template>
...
.suggestions{display:grid;gap:var(--space-2)}
.suggestions>span{color:var(--color-text-muted);font-size:var(--text-xs)}
.suggestions>div{display:flex;gap:var(--space-2);overflow-x:auto;padding-bottom:2px;scrollbar-width:thin}
