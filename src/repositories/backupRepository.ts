...
export function validateBackup(value:unknown):value is BackupData{
  ...
  const validSets=value.weightSets.every(x=>isObject(x)&&typeof x.id==='string'&&typeof x.weightLogId==='string'&&typeof x.weight==='number'&&typeof x.repetitions==='number'&&(x.durationSeconds===undefined||(typeof x.durationSeconds==='number'&&Number.isFinite(x.durationSeconds)&&x.durationSeconds>0&&x.repetitions===0))&&(x.durationUnit===undefined||x.durationUnit==='seconds'||x.durationUnit==='minutes'))
